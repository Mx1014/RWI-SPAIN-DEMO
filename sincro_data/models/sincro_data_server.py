...
class FullFabricServer(models.Model):
    ...
    def check_create_person_facts(self, *args):
        for server in self:
            ...
            fixed_msg, fixed_data = '', []
            adm_house_address = adm_family.home_address_id
            idx = 1
            fix_idx = 1

            # check the name family and email
            all_family_names = self._send_request(self.api_base_url + '/families?pageSize=%s' % MAX_INT,
                                                  'get',
                                                  headers, '{}')
            #
            ...
            if family_id != -1 and adm_family and str(family_id) != str(adm_family.facts_id):
                is_warning = True
                ...
                idx += 1
                new_name = self._generate_new_name(adm_family.name, 'name', 'res.partner')

                fixed_msg += '%s. Cambiar nombre de la familia con FACTS ID %s de "%s" a "%s"\n' % (
                    fix_idx, adm_family.facts_id, adm_family.name, new_name)
                fixed_data.append({'model': 'res.partner', 'item_id': adm_family.id,
                                   'field': 'name',
                                   'new_data': new_name})
                fix_idx += 1

                if not adm_family.facts_id:
                    fixed_msg += '%s. Asignar el FACTS ID %s a la familia %s en Odoo.\n' % (
                        fix_idx, family_id, adm_family.name)
                    fixed_data.append({'model': 'res.partner', 'item_id': adm_family.id,
                                       'field': 'facts_id',
                                       'new_data': family_id})
                    fix_idx += 1
            ...
            if adm_student.first_name and adm_student.last_name:
                res_check_student = self._send_request(
                    '%s/People?filters=firstName@=%s,middleName@=%s,lastName@=%s' % (
                        self.api_base_url, adm_student.first_name, str(self._clean_false_to_empty(
                            adm_student, ['middle_name'],
                            '')), adm_student.last_name),
                    'get',
                    headers, '{}')

                for facts_student in json.loads(res_check_student.text)['results']:
                    if str(facts_student['personId']) != adm_student.facts_id:
                        is_error = True
                        aux_cad = '%s. The student %s with FACTS_ID %s in Odoo has a potential duplicate person (%s)- in FACTS --> Facts Student: %s(%s).\n'
                        aux_tuple = (
                            idx, adm_student.name, adm_student.facts_id, adm_student.email,
                            '%s %s' % (facts_student['firstName'], facts_student['lastName']),
                            facts_student['personId'])

                        if not adm_student.facts_id:
                            aux_cad = '%s. The student %s without FACTS_ID in Odoo has a potential duplicate person (%s)- in FACTS --> Facts Student: %s(%s).\n'
                            aux_tuple = (
                                idx, adm_student.name, adm_student.email,
                                '%s %s' % (facts_student['firstName'], facts_student['lastName']),
                                facts_student['personId'])
                            fixed_msg += '%s. Se le puede asignar al estudiante %s el FACTS ID %s in Odoo.\n' % (
                                fix_idx, adm_student.name, facts_student['personId'])
                            fixed_data.append({'model': 'res.partner', 'item_id': adm_student.id,
                                               'field': 'facts_id',
                                               'new_data': facts_student['personId']})

                        fix_idx += 1
                        response_msg += aux_cad % aux_tuple
                        idx += 1
            ...
            if adm_student.email:
                ...
                for facts_student in json.loads(res_check_student.text)['results']:
                    if str(facts_student['personId']) != adm_student.facts_id:
                        is_error = True
                        aux_cad = '%s. The student %s with FACTS_ID %s in Odoo has a potential duplicate email (%s)- in FACTS --> Facts Student: %s(%s).\n'
                        aux_tuple = (
                            idx, adm_student.name, adm_student.facts_id, adm_student.email,
                            '%s %s' % (facts_student['firstName'], facts_student['lastName']),
                            facts_student['personId'])
                        ...
                        if not adm_student.facts_id:
                            aux_cad = '%s. The student %s without FACTS_ID in Odoo has a potential duplicate email (%s)- in FACTS --> Facts Student: %s(%s).\n'
                            aux_tuple = (
                                idx, adm_student.name, adm_student.email,
                                '%s %s' % (facts_student['firstName'], facts_student['lastName']),
                                facts_student['personId'])
                            ...
                            fixed_msg += '%s. Borrar el email del estudiante %s con FACTS ID %s \n' % (
                                fix_idx, adm_student.name, facts_student['personId'])
                            fixed_data.append({'model': 'res.partner', 'item_id': adm_student.id,
                                               'field': 'email',
                                               'new_data': False})
            ...
            for rel in adm_relationships:
                ...
                if person.email:
                    ...
                    for facts_parent in json.loads(res_check_parent.text)['results']:
                        if str(facts_parent['personId']) != person.facts_id:
                            is_error = True

                            aux_cad = '%s. The person %s with FACTS ID %s in Odoo has a potential duplicate email (%s) in FACTS --> Facts Person: %s(%s).\n'
                            aux_tuple = (
                                idx, person.name, person.facts_id, person.email,
                                '%s %s' % (facts_parent['firstName'], facts_parent['lastName']),
                                facts_parent['personId'])

                            if not person.facts_id:
                                aux_cad = '%s. The person %s without FACTS ID in Odoo has a potential duplicate email (%s) in FACTS --> Facts Person: %s(%s).\n'
                                aux_tuple = (
                                    idx, person.name, person.email,
                                    '%s %s' % (facts_parent['firstName'], facts_parent['lastName']),
                                    facts_parent['personId'])

                                fixed_msg += '%s. Se le puede asignar al pariente %s el FACTS ID %s in Odoo.\n' % (
                                    fix_idx, person.name, facts_parent['personId'])
                                fixed_data.append({'model': 'res.partner', 'item_id': person.id,
                                                   'field': 'facts_id',
                                                   'new_data': facts_parent['personId']})
                            else:
                                fixed_msg += '%s. Borrar el email del pariente %s con FACTS ID %s\n' % (
                                    fix_idx, person.name, facts_parent['personId'])
                                fixed_data.append({'model': 'res.partner', 'item_id': person.id,
                                                   'field': 'email',
                                                   'new_data': False})
            ...
            if adm_house_address:
                all_address_facts = self._send_request(self.api_base_url + '/people/Address?pageSize=%s' % MAX_INT,
                                                       'get',
                                                       headers, '{}')
                ...
                if address_facts_id != -1:
                    ...
                    family_members_of_family = []

                    if adm_family.facts_id:
                        persons_belongs_to_family_res = self._send_request(
                            '%s/People/PersonFamily?filters=familyid==%s' % (self.api_base_url, adm_family.facts_id),
                            'get',
                            headers, '{}')

                        family_members_of_family = list(map(lambda x: x['personId'],
                                                            json.loads(persons_belongs_to_family_res.text)['results']))

                    idx_inner = 1
                    has_address_error = False
                    for person in json.loads(persons_with_same_address.text)['results']:
                        if person['personId'] not in family_members_of_family:
                            if idx_inner == 1:
                                response_msg += '%s. The next persons has the same address and not belongs to the family %s in FACTS:\n' % (
                                    idx, adm_family.name)
        ...
        # check required field of the application
        required_fields_raw = application.get_required_fields()
        ...
        for checked_field in required_fields:
            if checked_field not in application or not application[checked_field] or application[checked_field] == '':
                is_warning = True
                response_msg += '%s. The student %s doesn´t have to assign a %s.\n' % (idx,
                                                                                       str(application.partner_id.name),
                                                                                       checked_field)
                idx += 1
        ...
        for server in self:
            ...
            if not existed_automation:
                existed_automation = automation_env.create({
                    ...
                    'filter_pre_domain': '[["status_id.import_to_facts","=",False]]',
