...
class ChangeState(models.Model):
            ...
            self.env['adm.application'].browse(item).with_context({'forcing': True}).write(
                {
                    ...
                    'fixed_msg': res['fixed_msg'],
                    'fixed_data': json.dumps(res['fixed_data'])
                })
            if max_columns < len(res['fixed_data']):
                max_columns = len(res['fixed_data'])

        self.env['adm.application'].browse(active_ids).with_context({'forcing': True}).write(
            {'column_available_adm': max_columns})
    ...
    def update_state_application(self):
        ...
        for record in self.env['adm.application'].search([('id', 'in', active_ids), ('is_error', '=', False)]):
