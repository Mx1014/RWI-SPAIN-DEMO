...
class Application(models.Model):
    ...
    def create(self, values):
        ...
        application_id._recompute_followers()
    ...
    def _recompute_followers(self):
        for application_id in self:
            partner_ids = application_id.mapped('responsible_user_kanban_ids.partner_id')
            partner_ids += application_id.responsible_user_id.partner_id
            partner_ids += application_id.user_id.partner_id
            partner_ids += application_id.mapped('partner_id.family_ids.member_ids').filtered_domain([('person_type', '=', 'parent')])

            application_id.message_subscribe(partner_ids=partner_ids.ids)
